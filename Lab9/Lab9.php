...
<body>
    ...
    <?php
        ...
        function ComputePizzaTotal($pizzaMenu, $toppingsMenu, $pizzaOrder, $toppings, $quantity) {
            $total = 0;
            //pizza Price
            if(isset($pizzaMenu[$pizzaOrder])) {
                $total += $pizzaMenu[$pizzaOrder];
            }

            //topping price
            if(!empty($toppings)) {
                foreach($toppingsMenu as $toppers) {
                    if(isset( $pizzaMenu[$toppers])) {
                        $total += $pizzaMenu[$toppers];
                    }
                }
            }
                ...
                <form method="post">
                    ...
                    <div class="form-group">
                        ...
                        <div class="radio-group">
                            <select name="pizza">
                                ...
                                foreach($pizzaMenu as $key => $value) { //select pizza option
                                    ?>
                    ...
                    <div class="form-group">
                        ...
                        <div class="checkbox-group">
                            <?php
                            $toppings = []; //toppings option
                            foreach($toppingsMenu as $key => $value) {
                                echo "<label>
            ...
            <div class="card">
                ...
                <?php
                    if (isset($_POST['order'])) {
                        ...
                        $toppingPizza = $_POST['toppings'] ??[ ];
                        $quantity = $_POST['qty'] ?? 1;

                        $fullTotal = ComputePizzaTotal($pizzaMenu, $toppingsMenu, $pizzaOrder, $toppingPizza, $quantity);
                        
                         echo "<div class='summary-item'><span>Name:</span>".htmlspecialchars($custName)."</div>";
                         echo "<div class='summary-item'><span>Pizza Order:</span>".htmlspecialchars($pizzaOrder);
                         echo " x" . htmlspecialchars($quantity)."</div>";

                         echo "<div class='summary-item'><span>Toppings:</span>";

                         foreach($toppingPizza as $key => $value) {

                            echo ''.htmlspecialchars($value) . ", ";
                         }
                         echo "</div>";

                         echo "<div class='total'>Ammount Due: ₱".htmlspecialchars($fullTotal)."</div>";
                ...
                <?php
                    if (isset($_POST['order'])) {
                        for($i = 1; $i <= $quantity; $i++) {
                            echo "<div class='order-counter'>Order #$i - $pizzaOrder</div>";
